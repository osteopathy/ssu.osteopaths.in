...
import { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI, GOOGLE_SCOPES, GOOGLE_CALENDAR_SCOPES } from "$env/static/private";
import { db } from "./db";
...
export default function calendarService(credential: { access_token: string; refresh_token: string | null; calendarId: string }) {
    const googleAuth = (credential: { access_token: string; refresh_token: string | null; calendarId: string }) => {
        ...
        const refreshAccessToken = async (myGoogleAuth: Awaited<ReturnType<typeof getGoogleAuth>>) => {
            try {
                ...
                await db.update(calendarTable).set({
                    accessToken: token.access_token,
                    refreshToken: token.refersh_token,
                    accessTokenExpiresAt: token.expiry_date,
                }).where(eq(calendarTable.id, credential.calendarId));
                myGoogleAuth.setCredentials({
                    access_token: credential.access_token,
                    refresh_token: credential.refresh_token,
