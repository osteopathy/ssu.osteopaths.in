...
export const actions: Actions = {
	...
	username: async (event) => {
		...
		const [res1, res2] = await Promise.allSettled([
			createUsername(username, osteopath.id, event.locals.user.id),
			db.update(osteopathTable).set({ username }).where(eq(osteopathTable.id, osteopath.id))
		]);
		...
		if (res1.status === 'rejected') {
			if (res2.status === 'fulfilled')
				await db
					...
					.set({ username: osteopath.username })
